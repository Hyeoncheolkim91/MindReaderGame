.include		"SP2019-CSSE3340-503-MindReaderBinaryManipulationMACROS-WeOverFlow.asm"
...
arrayElementCalculator:	
	...
	lw 	$t6, randInt($zero)	# Load random integer into $t6. $t6 holds the number of card that will be printed
...
checkCard:
	...
	Card_Number_Evaluation (0, 1, random)	# if card 0 has beend displayed, branch to "random" ; else display card.
		
	card1:						
	bgt 	$t7, 1, card2			# skip card 1
	Card_Number_Evaluation (4, 2, random)   # if card 5 has beend displayed, branch to "random" ; else display card.
	
	card2:
	bgt 	$t7, 2, card3			# skip card 2
	Card_Number_Evaluation (8, 4, random)	# if card 5 has beend displayed, branch to "random" ; else display card.
		
	card3:
	bgt 	$t7, 3, card4			# skip card 3
	Card_Number_Evaluation (12, 8, random)	# if card 5 has beend displayed, branch to "random" ; else display card.
	
	card4:
	bgt 	$t7, 4, card5			# skip card 4
	Card_Number_Evaluation (16, 16, random) # if card 5 has beend displayed, branch to "random" ; else display card.
	
	card5:	
	Card_Number_Evaluation (20, 32, done)	# if card 5 has beend displayed, branch to "done" ; else display card.
done:
